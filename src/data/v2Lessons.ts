...
export const v2Lessons: V2Lesson[] = [
  ...
  },
  {
    no: 5,
    phase: '확장하기',
    title: '진짜처럼 보이는 가짜를 찾아라, AI 탐정단',
    goal: 'AI가 만든 영상, 그림, 광고, SNS 글을 살펴보며 진짜처럼 보이는 콘텐츠를 비판적으로 판단한다.',
    screenCue: 'AI 가짜 의사 사례 + 실시간 AI 탐정 퀴즈 + 인쇄용 진짜 vs 가짜 카드',
    studentAction: 'AI 탐정단으로 참여해 의심할 점, 출처 확인, 과장 광고와 딥페이크 가능성을 판단한다.',
    mustKeep: '그럴듯하게 보인다고 바로 믿지 않고 출처, 의도, 행동 요구를 확인하는 태도를 잡는다.',
    teacherScript: [
      '유튜브 건강 광고와 AI 가짜 의사 사례를 보고 왜 특히 어르신들이 속기 쉬운지 이야기한다.',
      '학생들이 QR로 AI 탐정단 퀴즈에 참여해 어떤 콘텐츠를 의심해야 하는지 고른다.',
      '진짜 vs 가짜 정보 카드를 모둠별로 분류하며 어떤 근거로 판단했는지 말한다.',
    ],
  },
  {
    no: 6,
    phase: '확장하기',
    title: '자유 테스트 및 임명식',
    goal: '학생들이 자유롭게 질문을 시험하고, 완성된 가치코드를 바탕으로 학급 AI를 임명한다.',
    screenCue: '자유 테스트 채팅 + 가치코드 회고 + 임명식 화면',
    studentAction: '자유 테스트 질문 제안, 가치코드 최종 확인, AI를 잘 키우는 다짐 작성',
    mustKeep: '학생들이 만든 기준이 AI의 답변을 바꿨다는 성취감으로 마무리한다.',
    teacherScript: [
      '학생들이 직접 시험 질문을 내고 어떤 가치코드가 작동하는지 확인한다.',
      '부족한 기준이 있으면 마지막으로 보완한다.',
      '임명식에서 완성된 학급 AI와 가치코드를 함께 선언한다.',
    ],
  },
]
