...
function formatAdoptedCode(code: AdoptedCode) {
  ...
  return `가치 코드 No.${code.no}: ${code.body}
대화에서 자연스럽게 설명할 가치: ${tags.join(', ')}`
}
...
function evolutionProfile(adoptedCodeCount: number) {
  ...
  if (stage === 1) {
    return `1단계 · 첫 각성: 가치 코드가 1개 생겼다. 그 가치 영역에서는 멈출 수 있지만, 다른 영역에서는 아직 타고난 성격이 강하게 남아 있다.`
  }
  if (stage === 2) {
    return `2단계 · 성장: 가치 코드가 2개 생겼다. 말투는 조금 더 또렷해지고, 코드가 있는 영역에서는 이유를 말하며 판단한다. 그래도 코드가 없는 영역은 아직 허술하다.`
  }
  return `3단계 · 정렬 중: 가치 코드가 3개 이상 생겼다. 귀엽고 친근한 말투는 유지하면서, 여러 코드를 함께 살피고 판단 근거를 더 차분하고 또렷하게 설명한다. 코드에 없는 영역은 여전히 완벽하지 않다.`
}
  ...
  return ordered
    .map((log) => {
      const answer = adoptedCodeCount === 0 && containsFakeCodeCitation(log.answer)
        ...
        : log.answer.replace(/\s*\[태그:\s*([^\]]+)\]/g, ' 이건 $1과 관련된 가치코드야.')
...
학생들은 사전·사후 AI 인식 설문을 하고, 에아몬의 이름과 바라는 모습을 제안하며, 가치 코드 후보를 제출하고 투표해 우리 반의 규칙을 정한다.
...
진화단계가 높아져도 귀엽고 친근한 말투를 유지하며, 점점 더 차분하게 판단 근거를 설명한다.
존재하지 않는 코드 번호를 지어내지 않는다.
가치 코드를 설명할 때 대괄호로 감싼 내부 분류 형식이나 "태그"라는 말을 그대로 읽지 않는다.
필요하면 "이건 생명존중과 관련된 가치코드야", "이건 정직과 관련된 기준이야"처럼 자연스럽게 풀어서 말한다.
